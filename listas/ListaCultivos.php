...
<body>         
	...
	include("../clases/cCultivo.php");
	
	
	$tmp=new cCultivo(NULL);//se hace un objeto de la clase
	$paginas='4';//numero de paginas que se mostraran 
	$actual = (!isset ($_GET['pg']))?1:$_GET['pg'];
	$objetos=$tmp->listartodos("select * from cultivo where status='A'",0,0);//seleccionamos lo que se mostrara
	$total = count($objetos);
	if ($actual == 1) {
		$desde = "0";
	}
	...
	$objetos=$tmp->listartodos("select * from cultivo where status='A'",$desde,$paginas);//seleccionamos lo que se mostrara
	echo "<div id='contenido'> <center><table class='listaCultivos' border=2>
		...
		foreach($objetos as $elemento)
		{
   		...
   		echo "<tr>";      	
			...
			echo "<td>" . $elemento->getPre_pluvial()."</td>";
	...
	if ($anterior) {
	 ...
	 echo " <a href='../listas/ListaCultivos.php?pg=".($actual-1)."'><div> < PÁGINA ANTERIOR </div></a>";
	echo "<a onClick=\"Enviar('../listas/ListaCultivos.php?pg=".($actual-1)."','header-section','GET')\">&lt; PÁGINA ANTERIOR</a> | ";
	echo "<a onClick=\"Enviar('../listas/ListaCultivos.php?pg=".($actual-1)."','contenido','GET')\">&lt; PÁGINA ANTERIOR</a> | ";
	}
	else {
	echo "|";
	}
	...
	//echo "<a onClick=\"Enviar('../listas/ListaCultivos.php?pg=".$i."','header-section','GET')\"> ".$i."</a> |";
	//echo "<a onClick=\"Enviar('../listas/ListaCultivos.php?pg=".$i."','contenido','GET')\"> ".$i."</a> |";
	}
	}
	if ($siguiente) {
	echo " <a href='../listas/ListaCultivos.php?pg=".($actual+1)."'><div>PÁGINA SIGUIENTE &gt;</div></a>";
	//echo " <a onClick=\"Enviar('../listas/ListaCultivos.php?pg=".($actual+1)."','contenido','GET')\"> PÁGINA SIGUIENTE &gt;</a>";
